...
local function initializeAutoFish()
    ...
    local criticalModules = {"autofish", "orion_ui"}
    ...
    -- Initialize ORION UI last (it needs all other modules)
    if modules.orion_ui then
        modules.orion_ui.init(modules)
        
        -- Show welcome notification
        modules.orion_ui.showNotification(
            "Welcome!",
            "AutoFish Pro loaded successfully with ORION UI!",
            5
        )
    end
    
    print("✅ AutoFish Pro initialization complete!")
    print("🎮 Use the ORION UI interface to control all features")
